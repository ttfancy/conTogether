...
// LogRegistrar is the one thing this handler needs from *applog.Manager
...
type LogRegistrar interface {
	RegisterLogHandler(handler applog.LogHandler) (unregister func())
}
...
func ServeAppLogs(registrar LogRegistrar, authStore middleware.APIKeyStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		unregister := registrar.RegisterLogHandler(applog.LogHandlerFunc(func(e applog.LogEntry) {
			...
			default:
				...
				// applog write-loop every other log write also goes
